...
void BFS_Shortest(Graph g,int start, int stop , int * truoc)
{
  int visited[1000] = {};
...
int main()
{
    ...
    int truoc[100] = {-1};
    int start = 1, stop = 5;
    BFS_Shortest(g,start,stop,truoc);
    if(truoc[stop]!= -1)
    {
        printf("Duong di tu %d den %d la:",start, stop);
        int i= stop;
        do{
            printf("%d <--- ",i);
            i = truoc[i];
    ...
    {
        printf("Khong ton tai duong di tu %d den %d",start , stop);
    }
     dropGraph(g);
}